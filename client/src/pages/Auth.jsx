...
function RegisterDropdownBar({ inputs, onChange }) {
  const usedDigits = new Set(inputs.filter(v => v !== ""));

  return (
    <div className="reg-wrap">
      <div className="reg-header">
        {POSITIONS.map(p => (
          <div key={p} className="reg-head-cell">
            {p}
          </div>
        ))}
      </div>
...
export default function Auth() {
  ...
  const resetAll = useCallback((newMode) => {
    ...
    setSentence(""); setOffset(getRandomOffset());
setPositions(getRandomPositions());
  ...
  const handleSignup = async () => {
    ...
    if (isNaN(off) || off < 0 || off > 9) { setError("Offset must be 1–99."); return; }
  ...
  const handleVerify = async () => {
    setError("");
    if (!allRegFilled) { setError("Fill all 10 positions (A – E)."); return; }
    setLoading(true);
    try {
...
.reg-header,.reg-dropdowns{display:grid;grid-template-columns:repeat(5,1fr);min-width:360px;}
